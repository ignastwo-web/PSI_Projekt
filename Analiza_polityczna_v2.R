...
p_sentyment_slowa <- ggplot(top_sentyment, aes(x = freq, y = word2, fill = sentyment)) + 
  geom_col(show.legend = FALSE) +
  facet_wrap(~grupa + sentyment, scales = "free_y", ncol = 2) + 
  scale_y_reordered() + # POPRAWKA: Zostało tylko scale_y_reordered, bo słowa są na osi Y
  scale_fill_manual(values = c("pozytywny" = "darkolivegreen4",
                               "negatywny" = "firebrick")) +
  labs(
