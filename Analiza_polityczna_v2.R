...
# setwd("C:/Users/Lenovo/Desktop/projekt_PSI")  


# Wczytaj słownik sentymentu (plik musi być w katalogu roboczym):
slownik <- read.csv("slownik_polityczny_lematy.csv",
...
# Przy pierwszym uruchomieniu pobierz model (ok. 20 MB, tylko raz):
# udpipe_download_model(language = "polish-lfg")

# Wczytaj model (podaj ścieżke pobranego pliku z 67 wiersza (consola powinna podać ścieżke)):
...
# -- PiS u WŁADZY --
cat("\nPiS – władza:\n")
pis_wladza_pliki <- c("C:/Users/Lenovo/Desktop/projekt_PSI/Morawiecki za władzy.txt"
...
# -- PiS w OPOZYCJI --
cat("\nPiS – opozycja:\n")
pis_opozycja_pliki <- c("C:/Users/Lenovo/Desktop/projekt_PSI/Morawiecki przed władzą.txt"
...
# -- PO u WŁADZY --
cat("\nPO – władza:\n")
po_wladza_pliki <- c("C:/Users/Lenovo/Desktop/projekt_PSI/Tomasz Siemoniak za władzy.txt"
...
# -- PO w OPOZYCJI --
cat("\nPO – opozycja:\n")
po_opozycja_pliki <- c("C:/Users/Lenovo/Desktop/projekt_PSI/Tomasz Siemoniak przed władzą.txt"
...
top_slowa <- dane_wszystkie %>%
  group_by(grupa) %>%
  slice_max(order_by = freq, n = 20, with_ties = FALSE) %>% # POPRAWKA: with_ties=FALSE usuwa ścisk remisów 1-wystąpieniowych
  ungroup() %>%
  mutate(word2 = reorder_within(word, freq, grupa))

p_liczebnosc <- ggplot(top_slowa, aes(x = freq, y = word2, fill = grupa)) + # POPRAWKA: Rezygnacja z coord_flip na rzecz natywnego układu X/Y
  geom_col(show.legend = FALSE) +
  facet_wrap(~grupa, scales = "free_y", ncol = 2) + # POPRAWKA: scales="free_y" dedykowane osi pionowej
  scale_y_reordered() +
  ...
  theme_gdocs() + # POPRAWKA: Najpierw motyw bazowy...
  theme(          # ...potem modyfikacje wielkości czcionek i marginesów
...
print(p_liczebnosc)
ggsave("wykres_top20_slow.png", plot = p_liczebnosc, width = 11, height = 8, dpi = 300)
...
top_sentyment <- sentyment_dane %>%
  ...
  slice_max(order_by = freq, n = 10, with_ties = FALSE) %>% # POPRAWKA: Zabezpieczenie przed remisami
  ungroup() %>%
  mutate(word2 = reorder_within(word, freq, interaction(grupa, sentyment)))

p_sentyment_slowa <- ggplot(top_sentyment, aes(x = freq, y = word2, fill = sentyment)) + # POPRAWKA: Układ natywny bez coord_flip
  geom_col(show.legend = FALSE) +
  facet_wrap(~grupa + sentyment, scales = "free_y", ncol = 2) + # POPRAPWKA: Przejście na układ 2 kolumn dla czytelności ośmiu paneli
  scale_x_reordered() +
  scale_y_reordered() +
  scale_fill_manual(values = c("pozytywny" = "darkolivegreen4",
                               "negatywny" = "firebrick")) +
...
print(p_sentyment_slowa)
ggsave("wykres_sentyment_slowa.png", plot = p_sentyment_slowa, width = 10, height = 11, dpi = 300)
...
generuj_chmure <- function(dane_df, tytul, paleta = "Dark2") {
  ...
  # POPRAWKA: scale = c(2.2, 0.4) proporcjonalnie zmniejsza słowa, zapobiegając błędom pomijania długich wyrazów
  wordcloud(words = dane_df$word, freq = dane_df$freq,
            min.freq = 2, max.words = 50, scale = c(2.2, 0.4),
            colors = brewer.pal(8, paleta), random.order = FALSE)
  title(tytul)
}

# POPRAWKA: Otwarcie dużego, zewnętrznego okna graficznego chroni chmury przed duszeniem się w małym widoku Plots
if (.Platform$OS.type == "windows") windows(width = 11, height = 11)
