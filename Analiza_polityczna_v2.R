...
generuj_chmure <- function(dane_df, tytul, paleta = "Dark2") {
  ...
  kolory_ciemne <- brewer.pal(8, paleta)[4:8]
  
  par(mar = c(0, 0, 2.5, 0))
  
  wordcloud(words = dane_df$word, freq = dane_df$freq,
            min.freq = 1, max.words = 50, scale = c(2.3, 0.4),
            colors = kolory_ciemne, random.order = FALSE)
  
  title(tytul, line = 0.5, cex.main = 1.2, font.main = 2)
}
